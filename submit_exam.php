...
// Get active attempt first (needed for both grading and security logging)
...
$has_valid_proctoring_session = isset($_SESSION['proctoring_enabled'])
    && ((int)($_SESSION['proctoring_exam_id'] ?? 0) === (int)$exam_id);

// If proctoring is required, enforce valid proctoring session
if ($requires_proctoring && !$has_valid_proctoring_session) {
    $stmt = $pdo->prepare(
...
$has_payload = !empty($answers_input) || !empty($theory_answers) || !empty($fill_in_answers);
...
// Fetch questions and scoring metadata
